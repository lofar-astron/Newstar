...
C
C  Revisions:
C	WNB 931214	Allow P:
C
	SUBROUTINE NGCXMH(PTYPE,INFCA,NGFP,SNAM)
C
C  Show plot header
C
C  Result:
C
C	CALL NGCXMH ( PTYPE_J:I, INFCA_J:I, NGFP_J:I, SNAM_J(*):I)
C					Show on output PTYPE the plot at NGFP
C					of file INFCA.
C	CALL NGCEMH ( PTYPE_J:I, INFCA_J:I, NGFP_J:I, SNAM_J(*):I)
C					Edit plot header
C
C  Include files:
C
	INCLUDE 'WNG_DEF'
	INCLUDE 'NGF_O_DEF'		!PLOTHEADER
	INCLUDE 'GFH_O_DEF'
	INCLUDE 'SGH_O_DEF'
	INCLUDE 'NGF_E_DEF'		!EDIT INFORMATION
	INCLUDE 'GFH_E_DEF'
	INCLUDE 'SGH_E_DEF'
C
C  Parameters:
...
C
	...
	BYTE NGF(0:NGFHDL-1)		!MAP HEADER
	  BYTE GFH(0:GFHHDL-1)
	  BYTE SGH(0:SGHHDL-1)
	  EQUIVALENCE (NGF,GFH,SGH)
...
C GET HEADER
C
	IF (.NOT.WNFRD(INFCA,NGFHDL,NGF,NGFP)) THEN
...
C SHOW HEADER
C
	CALL WNCTXT(PTYPE,'!/Plot header description !AS\:!/',
	1		WNTTSG(SNAM,0))
	CALL NSCXXS(PTYPE,NGF,NGFEDL,NGF_EC,NGF_EJ)	!ACTUAL SHOW
C
	RETURN
C
C NGCESH
C
	ENTRY NGCEMH(PTYPE,INFCA,NGFP,SNAM)
C
C INIT
C
	DEP=0					!CURRENT DEPTH
	CHP=NGFP				!HEADER POINTER
	CTYP=1					!CURRENT TYPE (NGF)
	CEDP=-1					!CURRENT POINTER IN EDIT LIST
	CHDL=NGFHDL				!CURRENT LENGTH
C
C ACTION
C
 10	CONTINUE
	DO WHILE (CTYP.GT.0)			!SOMETHING TO DO
	  IF (CHDL.LE.0) THEN			!GET NEW HEADER
	    IF (PLEN(0,CTYP).GE.0 .AND. CEDP.GT.0) THEN
	      CHDL=WNGGJ(NGF(PLEN(0,CTYP)))	!LENGTH FROM FILE
	    ELSE
...
C
	...
	1		(CTYP.LT.4 .OR. CTYP.GT.10)) THEN !MUST BE GFH
	    ...
	    CEDP=-1
	  END IF
	  CALL WNGMVZ(PLEN(1,CTYP),NGF)		!CLEAR BEFORE READ
	  IF (.NOT.WNFRD(INFCA,CHDL,NGF,CHP)) THEN
	    CALL WNCTXT(PTYPE,'Read error on input node')
	    RETURN
	  END IF
C
...
C
	  ...
	  DEPAR(4,DEP)=CHDL
	  IF (CTYP.EQ.1) THEN
	    CALL NSCXES(PTYPE,NGF,NGFEDL,NGF_EC,NGF_EJ,PLIST,
	1		CTYP,CEDP,CHPT,PSZ)
	  ELSE IF (CTYP.EQ.2) THEN
	    CALL NSCXES(PTYPE,NGF,GFHEDL,GFH_EC,GFH_EJ,PLIST,
	1		CTYP,CEDP,CHPT,PSZ)
	  ELSE IF (CTYP.EQ.3) THEN
	    CALL NSCXES(PTYPE,NGF,SGHEDL,SGH_EC,SGH_EJ,PLIST,
	1		CTYP,CEDP,CHPT,PSZ)
	  ELSE IF (CTYP.GE.4 .AND. CTYP.LE.10) THEN
	    CALL NSCXES(PTYPE,NGF,D_GEDL,
	...
	1		CTYP,CEDP,CHPT,PSZ)
	  ...
	  IF (CTYP.GE.4 .AND. CTYP.LE.10) THEN
	    ...
	    D_G_EJ(2,CTYP-3)=MAX(1,MIN(PSZ(1),NGFHDL/LB_Y)) !MAX. NUMBER TO DO
	  END IF
C
C REWRITE HEADER
C
	  IF (.NOT.WNFWR(INFCA,CHDL,NGF,CHP)) THEN
 30	    CONTINUE

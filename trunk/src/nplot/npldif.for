...
C	JPH 960730   Add sort for IFR_MODE = INVERT
C
C
	SUBROUTINE NPLDIF (LDATTP,IPOL,NHV)
C
C  Load data in buffer as (HA,IFR)
C
C  Result:
C
C	CALL NPLDIF		Load data in buffer as (HA,IFR)
C
C  Include files:
C
	INCLUDE 'WNG_DEF'
	...
	NPLOT=0						!SELECTED CIFRS
 	...
 	ELSE                                            !FIND INTERF. to DO
	  DO I=0,STHTEL-1
	    ...
	    END DO
	  END DO
	  IF (IFR_MODE.EQ.'INVERT') THEN 		!SORT ON East tel
	    DO I=0,NPLOT-2
	      DO I1=0,NPLOT-2-I
	        IF (BTEL(0,I1  )+STHTEL*BTEL(1,I1  ) .GT.
	1	    BTEL(0,I1+1)+STHTEL*BTEL(1,I1+1)) THEN    !SWAP
		  R0=BASEL(I1)
		  BASEL(I1)=BASEL(I1+1)
		  BASEL(I1+1)=R0
		  DO I2=0,1
...
C
 	IF (OPT.EQ.'TEL') THEN                          !FIND TELESCOPES TO DO
	   CALL NPLTEL (LDATTP,IPOL,NPLOT,PTXT,NHV,TABIFR,I1,I1)
	ELSE			                        !FIND REDUNDANT INTERF.
	   CALL NPLRES (LDATTP,IPOL,NPLOT,PTXT,NHV,TABIFR,I1,I1)
	ENDIF
C
	END

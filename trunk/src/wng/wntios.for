...
C
	DO IF=1,NFILE					!.DEF, _BD, _O, _T, _E
	  ...
	  IF (IF.EQ.1) THEN
	    IF (.NOT.DEFSN) THEN
	      IF (.NOT.PARSN .OR. BEGSN) GOTO 100 !NO .DEF
	      DOPAR=.TRUE.
	    ELSE
...
C
	    IF (IF.EQ.4) THEN
	      CALL WNCTXT(F(I),'!AS',CTXT(0,I,0))
	      CALL WNCTXT(F(I),'!AS!_Specification of translation tables:',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS',CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!_$ 0= end of table!40C$ 1= character',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),
	1	'!AS!_$ 2= 16 bits integer!40C$ 3= 32 bits integer',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!_$ 4= 32 bits real!40C$ 5= 64 bits real',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!_$ 6= repeat!40C$ 7= end repeat',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!_$ 8= undefined!40C$ 9= byte',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!_$10= external repeat!40C$11= start union',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!_$12= start map!40C$13= end union',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),
	1	'!AS!_$14= 64 bits complex!40C$15= 128 bits complex',
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!79C!AS',CTXT(0,I,1),
	1		CTXT(1,I,0))
	    END IF
...
C
	...
	1		CTXT(0,I,1))
	      CALL WNCTXT(F(I),'!AS!_$fieldname, pattern, units, special code',
	1		CTXT(0,I,1))
...
C
	...
	1		CTXT(1,I,0))
	          IF (I.EQ.0) THEN			!FORTRAN
	            CALL WNCTXT(F(I),'!@INTEGER !AS$HDL,!AS$HDV,!AS$HDS',
	1			LNAM1,LNAM1,LNAM1)
		    CALL WNCTXT(F(I),'!@  PARAMETER (!_!AS$HDL=!UJ,'//
	1			'!#C!! Length!/!@&!2_!AS$HDV=!AS,'//
	1			'!#C!! Version!/!@&!2_!AS$HDS=!AS)'//
	1			'!#C!! System',
	...
	1			'!#C!! Length!/!@&!2_!AS__V=!AS,'//
	1			'!#C!! Version!/!@&!2_!AS__S=!AS)'//
	1			'!#C!! System',
	1			LNAM1,FENTJ(WNTF_TLEN_J),COMPOS,
	1			LNAM1,PARM(P_VER),COMPOS,
	1			LNAM1,PARM(P_SYS),COMPOS)
...
C
	...
	1		CTXT(1,I,0))
		END DO
		CALL WNCTXT(F(1),'#ifdef wn_cv__!/$'//
	1		'#define !AS_com_ _!AS_com_!/$'//
	1		'#endif',LCNAM,LCNAM)
